...
const ndbinfoDiskpagebuffersQuery = `
    SELECT node_id, thr_no, block_name, thread_name, pages_written,
...
var (
	ndbinfoDiskpagebuffersPagesWrittenDesc = prometheus.NewDesc(
		...
		[]string{"nodeID", "threadNO", "blockName", "threadName"}, nil,
	)
...
func (ScrapeNdbinfoDiskpagebuffers) Scrape(ctx context.Context, db *sql.DB, ch chan<- prometheus.Metric) error {
	...
	for ndbinfoDiskpagebuffersRows.Next() {
		...
		ch <- prometheus.MustNewConstMetric(
			...
			strconv.FormatUint(nodeID, 10), strconv.FormatUint(threadNO, 10), blockName, threadName)
		ch <- prometheus.MustNewConstMetric(
		...
		ch <- prometheus.MustNewConstMetric(
			...
			strconv.FormatUint(nodeID, 10), strconv.FormatUint(threadNO, 10), blockName, threadName)
		ch <- prometheus.MustNewConstMetric(
			ndbinfoDiskpagebuffersIODesc, prometheus.CounterValue, float64(pageRequestsWaitQueue),
			strconv.FormatUint(nodeID, 10), strconv.FormatUint(threadNO, 10), blockName, threadName)
	}
	return nil
}
